...
export const formatMovies = (movies: IMovie[]): Partial<IMovie>[] => {
  ...
  let myMovies = movies.filter((movie) => {
    ...
    const regExpIsYoutubeLink =
      /^[https://]+(www\.youtube\.com\/watch\?v=)?(youtu\.be\/)?[-\d\w]*/;
...
export const dCheckIsSaved = (movies: IMovie[], savedMovies: IMovie[]) =>
  movies.map((m) => {
    const movie = { ...m };
    movie.isSaved = false;

    savedMovies.forEach((sm) => {
      if (movie.movieId === sm.movieId) {
        movie.isSaved = true;
        movie._id = sm._id;
      }
    });
...
export const dFilledLikeMovies = (movies: IMovie[], movie: IMovie): IMovie[] =>
  movies.map((m) => {
    const mm = { ...m };
    if (mm.movieId === movie.movieId) {
      mm.isSaved = true;
      mm._id = movie._id;
    }
    return mm;
...
export const removedLikeMovies = (movies: IMovie[], movie: IMovie): IMovie[] =>
  movies.map((m) => {
    if (m.movieId === movie.movieId) m.isSaved = false;
    return m;
  });

export const dRemovedLikeMovies = (movies: IMovie[], movie: IMovie): IMovie[] =>
  movies.map((m) => {
    const mm = { ...m };
    if (mm.movieId === movie.movieId) mm.isSaved = false;
    return mm;
  });
